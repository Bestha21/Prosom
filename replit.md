...
- **Access Control**: Passport.js with OIDC strategy and `isAuthenticated` middleware for route protection. Role-based access control with roles: employee, hr_manager (full HR module access), leadership (org tree, department-specific access), manager (team-specific access, all basic info), asset_team, payroll_team, project_team, onboarding_team, pms_team, lms_team, admin (full system access). Multiple roles can be combined comma-separated.
...
- **Bulk Employee Update**: CSV-based bulk update for employee records. Upload CSV, select match field (Employee Code or Email), choose which fields to update, preview changes, and apply. Server-side field whitelist prevents privilege escalation. Max 1000 rows per batch. Accessible to admin, HR, and HR Manager roles.
